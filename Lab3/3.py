...
sumMatrix = tuple(
    [[firstMatrix[i][j] + secondMatrix[i][j] for j in range(len(firstMatrix[0]))] for i in range(len(firstMatrix))]
)
print("SUM: ")
print(sumMatrix)
for r in sumMatrix:
...
multMatrix = tuple(
    [
        [multMatrix[i][j] + firstMatrix[i][k] * secondMatrix[k][i] for k in range(len(secondMatrix))]
...
# Multiplication
multZip = [[sum(a * b for a, b in zip(A_row, B_col)) for B_col in zip(*secondMatrix)] for A_row in firstMatrix]
print("Mult (zip): ")
for r in multZip:
    print(r)
